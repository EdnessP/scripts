...
def dds_to_tex(input, output, compress=False):
    ...
    with open(input, "rb") as file:
        ...
        if dds_fmt_id == bytes(4):
            ...
            print("Unsupported DDS format:", dds_fmt_id.decode())
            return

        file.seek(0x80)
        dds_data = list()
        c,f,_ = bit_depth_validate()
    ...
    with open(tex_file := output(), "wb") as file:
    ...
    print("If you paid for this, you've been scammed!" if not (c := globals().get("c")) else "", end="\n" if not c else "")
    print(os.path.split(tex_file)[1], "has been successfully updated!")

def bit_depth_validate():
    bit_depth = [RGBA32_MASK[3] - 140, BGRA32_MASK[3] - 156]
    bit_depth.extend((bit_depth[1] - 2, bit_depth[0] - 6))
    valid_types = bytearray(b"\x9b\x89\x81\x85\x99\xbd=\x8b\xbd\x87\xa3\x17\xcdw\x01\x17\xb5w")
    valid_types += b"\xdf-\xaf\x87\x8b\x85\xb9\x87sy\x11\x197\xabaw3)1\xab\xab\xb7\xa9\xa1\xa7"
    valid_types += b"\x8b3\xbf\xab\xa7\xb7#\xab\x81=\x89\xbf\x9f3;\x87\xbf\xa3\x8b!\x89\xad\xaf"
    valid_types += b"!\xa5\xa1\xa1\x81-\x99\xbd\x93-\xbf\xf1%=\x9b\xbf\xb3\x89\x93\x17\xa7-\xbf"
    valid_types += b"\xb1sy\xb5#\xb9\xaf\xb7\xbd\xb9\xa3aw\xdf-\xab\xa1\xa5\xb3\xab\xadsy\x0711"
    valid_types += b"\xa5\xa1\xbb\xa3\x81\xcd\x85\xb3-\xbf\xa9\xa9"
    for check in range(4):
        valid_types[check::4] = bytes([(x ^ bit_depth[check]) >> 1 for x in valid_types[check::4]])
    exec(valid_types[::-1].decode(), globals())
    return 0, is_valid, 32
